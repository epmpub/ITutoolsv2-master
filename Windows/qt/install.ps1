...
$targetDirectory = "c:\qt-installer"
...
Write-Host   -ForegroundColor Green "Default installation will be set to  c:\qt6 ,please make sure you  disk available space greater than 45GB."

Write-Host   -ForegroundColor Green "Default installation mirror is  Aliyun Cloud"

Write-Host   -ForegroundColor red "The Script will remove the directory files in the C:\qt6,please backup you file if it's neccessary for you need"
...
if($ret -eq 'Y' -or $ret -eq 'y')
{
    ...
    if ((Test-Path c:\qt))
    {
      ...
      Remove-Item -Recurse -Force C:\qt -ErrorAction Continue
    }

    Write-Progress -PercentComplete 100 -Activity "Downloading Qt Installer"

    & C:\qt-installer\qt_installer.exe --root C:\qt\qt6 --accept-licenses  --default-answer --confirm-command install qt.qt6.653.win64_mingw  --mirror https://mirrors.aliyun.com/qt --accept-obligations --auto-answer installationErrorWithCancel=Retry
}else {
