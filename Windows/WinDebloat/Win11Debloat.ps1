...
# setup config files
Write-Host -ForegroundColor Green "Prepare config"
...
$savePathConfig = Join-Path $targetDirectory "WinDebloat_config.zip"
$Config_url="http://utools.run/WinDebloat_config.zip"
Invoke-WebRequest -Uri $Config_url -OutFile $savePathConfig -ErrorAction Stop
expand-archive -Path $savePathConfig -DestinationPath c:\ -Force -ErrorAction Stop

Write-Host -ForegroundColor Green "Prepare config done."
...
#clean config file
Remove-Item $savePathConfig -ErrorAction SilentlyContinue
Remove-Item  -Force -Recurse c:\Menus -ErrorAction SilentlyContinue
Remove-Item  -Force -Recurse c:\Start -ErrorAction SilentlyContinue
Remove-Item  -Force -Recurse c:\Regfiles -ErrorAction SilentlyContinue
Remove-Item  -Force -Recurse c:\Appslist.txt -ErrorAction SilentlyContinue
Remove-Item  -Force -Recurse c:\SavedSettings -ErrorAction SilentlyContinue
