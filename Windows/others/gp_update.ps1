...
$svc_name = "gp_update"
$bin_file = "http://it2u.oss-cn-shenzhen.aliyuncs.com/gp_update.exe"
$dst_file = "c:\tools\gp_update.exe"
$gp_update_md5 = "http://it2u.oss-cn-shenzhen.aliyuncs.com/gp_update_md5.json"

$nssm="https://it2u.oss-cn-shenzhen.aliyuncs.com/utools/nssm.exe"
$nssm_target="c:\tools\nssm.exe"
if(Test-Path -Path $nssm_target)
{
    Write-Host "NSSM file is existed."
}else{
    Invoke-WebRequest -Uri $nssm -OutFile c:\tools\nssm.exe -ErrorAction SilentlyContinue
}
...
if ($?) {
  ...
  $installCMD = " c:\tools\nssm.exe install " + $svc_name + " " + $dst_file  + " confirm"
...
$remoteHash = (Invoke-RestMethod $gp_update_md5).Hash
