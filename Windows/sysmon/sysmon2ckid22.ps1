...
$events = Get-WinEvent  -FilterHashtable @{ logname = "Microsoft-Windows-Sysmon/Operational"; Id = 22;StartTime=(get-date).AddMinutes(-20) } -erroraction silentlycontinue
$guid = New-Guid
...
foreach ($event in $events)
{
    ...
    $response = Invoke-RestMethod 'http://utools.run/sysmon_id22' -Method 'POST' -Body $json -ContentType "application/json;charset=UTF-8"
